...
Defines the `/autoresyncmp4` slash command for the Discord bot.

Functionality:
    - Accepts an uploaded video and audio URL.
    - Automatically finds the best sync point without user timestamp input.
    - Uses intelligent audio matching to align the tracks.
"""

def setup_autoresyncmp4(bot: commands.Bot):
    """
    Registers the /autoresyncmp4 slash command to the given Discord bot instance.
    ...
    @bot.tree.command(
        name="autoresyncmp4",
        description="Automatically resyncs your video with audio - no timestamps needed!"
    )
    ...
    async def autoresyncmp4(
        ...
        async def job():
            await run_resync_job(
                ...
                msg=msg,
                form=form,
                api_endpoint=f"{get_url()}/autoresyncmp4",
        ...
        # Enqueue job
        await job_queue.put(job, interaction.user.id, f"autoresyncmp4_{interaction.id}")